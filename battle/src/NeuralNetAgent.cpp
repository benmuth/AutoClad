...
void NeuralNetAgent::playoutBattle(BattleContext &bc, std::stringstream* snapshot) {
    // Copy the playout pattern from SimpleAgent2
    while (bc.outcome == Outcome::UNDECIDED) {
        // Optional snapshot for debugging  
        if (snapshot != nullptr && bc.inputState == InputState::PLAYER_NORMAL) {
            *snapshot << "Turn: " << bc.turn << ", HP: " << bc.player.curHp << "/" << bc.player.maxHp << std::endl;
        }
